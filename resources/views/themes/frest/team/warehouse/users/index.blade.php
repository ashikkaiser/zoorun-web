...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom d-flex">
            <h5 class="card-title">Warehouse Users List</h5>
            <div class="card-header-elements ms-auto">
                <a href="{{ route('admin.team.warehouse.user') }}"
                    class="btn btn-sm {{ request()->status !== 'trash' ? 'btn-dark' : '' }}"><span
                        class="tf-icon bx bx-check bx-xs"></span>
                    Active</a>
                <a href="{{ route('admin.team.warehouse.user', ['status' => 'trash']) }}"
                    class="btn btn-sm {{ request()->status === 'trash' ? 'btn-dark' : '' }}"><span
                        class="tf-icon bx bx-trash bx-xs"></span>
                    Trash</a>
                <a href="{{ route('admin.team.warehouse.user.create') }}" type="button"
