            ...
            <ul class="nav nav-pills mb-3 nav-fill card-header" role="tablist">
                <li class="nav-item">
                    <button type="button" class="nav-link tabbtn active" role="tab" data-bs-toggle="tab"
                        data-id="running" data-bs-target="#running-pickup" aria-controls="running-pickup"
                        aria-selected="false">
                        <i class="tf-icons bx bx-user"></i> Running Pickup
                    </button>
                </li>
                <li class="nav-item">
                    <button type="button" class="nav-link tabbtn " role="tab" data-bs-toggle="tab"
                        data-bs-target="#pickup-request" aria-controls="pickup-request" aria-selected="true"
                        data-id="pending">
                        <i class="tf-icons bx bx-home"></i> Pickup Requests
...
@section('inline-js')
    ...
    <script>
        $('#pickup-table').on('click', '.confirm-modal', function() {
            var parcel_id = $(this).attr('parcel_id');
            var url = "{{ route('rider.parcel.confirmParcel', ':parcel_id') }}";
            url = url.replace(':parcel_id', parcel_id);
    ...
    <script>
        $('#pickup-table').on('click', '.reschedule-modal', function() {
            var parcel_id = $(this).attr('parcel_id');
            var url = "{{ route('rider.parcel.rescheduleParcel', ':parcel_id') }}";
            url = url.replace(':parcel_id', parcel_id);
