...
use App\DataTables\Warehouse\SendTransfarDataTable;
...
{
    public function send(SendTransfarDataTable $dataTables, Request $request)
    {
        $warehouses = Warehouse::whereNot('branch_id', Auth::user()->branch_id)->get();
        $branches = Branch::whereNotIn('id', [Auth::user()->warehouse->id])->get();
        ...
        //         ->make(true);
        // }
        return $dataTables->render('themes.frest.warehousePanel.transfar.index', compact('warehouses', 'branches',));
    }
    ...
    public function sendParcel(Request $request)
    {
        ...
        $parcel->warehouse_id = $request->warehouse_id;
        $parcel->save();

        return response()->json([
            'status' => 'success',
            'message' => 'Parcel send to warehouse successfully',
        ]);
    }
